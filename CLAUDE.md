...
- Flutter `live_voice_screen.dart`: `_finishTurn` consumes the stream and queues each audio segment onto a sequential `playChain` (`Future.then`) so segment N plays right after N-1 finishes while later segments still stream in — first sentence starts almost immediately. `say` frames are collected and spoken via `_speakOnDevice` fallback at the end. Silence-detection tuning from the prior build kept (`_silenceHoldMs` 800, `_maxSpeechMs` 14000).
